...
router.get('/payroll/payments', async (req, res) => {
  try {
    ...
    const { data: lines } = await supabaseAdmin.from('finance_payroll_line')
      .select('employee_id, employee_name, salary, cnss_amo, ir, net_salary, paid, paid_date, payment_method, run_id').in('run_id', ids);
    const payments = (lines || []).map((l) => {
      ...
      return {
        ...
        paid: l.paid, paid_date: l.paid_date, payment_method: l.payment_method,
      };
