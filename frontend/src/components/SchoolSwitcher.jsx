...
const SchoolSwitcher = ({ className = '' }) => {
  ...
  const handlePick = async (id) => {
    ...
    try {
      ...
      alert(e.message || t('header.schoolSwitchFailed'));
      setBusy(false);
    }
  };
  ...
  return (
    <div className={`relative ${className}`} ref={ref}>
      ...
      >
        ...
        <span className="truncate">{school?.name || t('header.school')}</span>
        <ChevronDown className="w-3.5 h-3.5 text-muted-foreground shrink-0" />
      </button>

      {open && (
        <div className={`absolute ${dir === 'rtl' ? 'left-0' : 'right-0'} mt-1 w-64 rounded-xl border border-border bg-card shadow-lg z-50 overflow-hidden`}>
          <div className="px-3 py-2 text-xs font-semibold text-muted-foreground border-b border-border">{t('header.schools')}</div>
          {availableSchools.map((s) => (
            <button
              key={s.id}
              type="button"
              onClick={() => handlePick(s.id)}
              className="flex items-center gap-2 w-full px-3 py-2.5 text-sm hover:bg-accent text-start"
            >
              {s.logo_url
