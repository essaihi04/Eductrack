...
export default function TeacherAppointmentsPage() {
  ...
  const propose = async (a) => {
    ...
    if (!draft.proposed_at) return alert('Choisissez une date et une heure.');
    setBusy(a.id);
    try {
  ...
  const declare = async (a) => {
    const note = prompt('Message pour l\'administration (facultatif) :', '');
    if (note === null) return;
    setBusy(a.id);
    try {
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <CalendarClock className="w-6 h-6" /> Rendez-vous avec les parents
        </h1>
        <p className="text-sm text-gray-500 max-w-2xl">
          Proposez la date et l'heure qui vous conviennent. L'administration valide le créneau
          puis prévient le parent. Vous pouvez aussi répondre directement au message WhatsApp
          reçu (ex. « jeudi 10h30 »).
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
          ...
          <p className="text-gray-600">Aucune demande de rendez-vous</p>
        </div>
      ) : (
        <>
          <section className="space-y-3">
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">À traiter ({pending.length})</h2>
            {pending.length === 0 && <p className="text-sm text-gray-500">Rien en attente. 👌</p>}
            {pending.map((a) => {
                ...
                <div key={a.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 flex items-center gap-1 mt-0.5">
                        ...
                        {personName(a.parent) || 'Parent'}
                        {a.student ? ` — élève : ${personName(a.student)}` : ''}
                  ...
                  {a.status === 'propose' && a.proposed_at && (
                    <p className="text-sm text-blue-700 mt-2">
                      Vous avez proposé : <strong>{formatSlot(a.proposed_at)}</strong> — en attente de validation par l'administration.
                    </p>
                  )}
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-[220px_1fr_auto_auto] items-center">
                    ...
                    >
                      <Send className="w-4 h-4" /> {a.status === 'propose' ? 'Modifier' : 'Proposer'}
                    </button>
                    <button
                      onClick={() => declare(a)}
                      disabled={busy === a.id}
                      ...
                      title="L'administration reprendra la main"
                    >
                      Indisponible
                    </button>
                  </div>
