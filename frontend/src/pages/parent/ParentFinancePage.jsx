  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      <div className="rounded-xl bg-gradient-to-br from-emerald-600 to-teal-600 text-white p-6 shadow-lg mb-6">
        <h1 className="text-2xl font-bold flex items-center gap-2"><Wallet className="w-7 h-7" /> Finance</h1>
        <p className="text-white/80 text-sm">Factures et situation de paiement de vos enfants</p>
      </div>
      ...
      {children.length === 0 ? (
        ...
        <>
          ...
          {loadingInv ? (
            <div className="flex items-center justify-center h-40 text-gray-500">Chargement des factures…</div>
          ) : (
            <>
              {summary && (
                <div className="grid grid-cols-3 gap-3 mb-5">
                  <SummaryCard label="Total facturé" value={fmtMoney(summary.total, cur)} icon={FileText} color="text-blue-600" />
                  <SummaryCard label="Payé" value={fmtMoney(summary.paid, cur)} icon={CheckCircle2} color="text-green-600" />
                  <SummaryCard label="Reste à payer" value={fmtMoney(summary.remaining, cur)} icon={AlertCircle} color={summary.remaining > 0 ? 'text-red-600' : 'text-green-600'} />
                </div>
              )}
                          ...
                          <ul className="mt-3 text-sm text-gray-600 space-y-1">
                            {inv.lines.map((l) => (
                              <li key={l.id} className="flex justify-between">
                                <span>{l.label || l.description || 'Ligne'}</span>
