...
import { Plus, Trash2, ChevronDown, ChevronUp, RefreshCw, Copy, Eye, EyeOff, CheckCircle, User, Upload, Download, FileSpreadsheet, AlertCircle, Send, Edit2, X, KeyRound, BookOpen, LayoutGrid, Phone, Camera, FileText, ExternalLink } from 'lucide-react';
      ...
      <div className="border-t border-gray-200 pt-3">
        ...
        <p className="text-xs text-gray-400 mt-2">
          {hourly
            ...
            : 'Les matières enseignées, la photo et les documents (CIN, diplôme, contrat) se gèrent sur la fiche du prof.'}
        </p>
      </div>
    </div>
  );
            ...
            {teachers.length === 0 ? (
              ...
              teachers
                ...
                <div key={teacher.id} className="contents">
                  ...
                  >
                    <div className="space-y-3">
                      {/* En-tête fiche : avatar (modifiable) + matière + bouton modifier */}
                      <div className="flex items-center gap-3">
                        <TeacherPhoto teacher={teacher} apiUrl={apiUrl} onChange={fetchData} />
                      ...
                      <TeacherDocuments teacherId={teacher.id} apiUrl={apiUrl} />
...
// Jeton d'accès courant (même source que le reste de la page).
async function getToken() {
  const { data: { session } } = await (await import('../../lib/supabase')).supabase.auth.getSession();
  return session?.access_token;
}

// Photo de profil du prof (upload / suppression) — écrit dans profiles.avatar_url.
function TeacherPhoto({ teacher, apiUrl, onChange }) {
  ...
  const upload = async (file) => {
    ...
    try {
      ...
      const r = await fetch(`${apiUrl}/api/admin/teachers/${teacher.id}/photo`, {
  ...
  return (
    <div className="shrink-0 text-center">
      ...
      <label className={`mt-1 inline-flex items-center gap-1 text-xs cursor-pointer ${busy ? 'opacity-50' : 'text-blue-600 hover:text-blue-800'}`}>
        <Upload className="w-3 h-3" /> {teacher.avatar_url ? 'Changer' : 'Photo'}
        <input type="file" accept="image/*" className="hidden" disabled={busy} onChange={e => upload(e.target.files?.[0])} />
      </label>
      {teacher.avatar_url && <button onClick={remove} disabled={busy} className="block mx-auto text-xs text-gray-400 hover:text-red-500">Retirer</button>}
    </div>
  );
}

const TEACHER_DOC_TYPES = [['diploma', 'Diplôme'], ['cin', 'CIN'], ['contract', 'Contrat'], ['cv', 'CV'], ['other', 'Autre']];

// Documents administratifs du prof (CIN, diplôme, contrat…).
function TeacherDocuments({ teacherId, apiUrl }) {
  ...
  const load = async () => {
    try {
      const token = await getToken();
      const r = await fetch(`${apiUrl}/api/admin/teachers/${teacherId}/documents`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const add = async () => {
    ...
    try {
      ...
      fd.append('file', file); fd.append('doc_type', docType); fd.append('label', label);
      const r = await fetch(`${apiUrl}/api/admin/teachers/${teacherId}/documents`, {
        method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: fd,
      });
      if (!r.ok) throw new Error((await r.json().catch(() => ({}))).error || 'Échec de l\'envoi');
      setLabel(''); setFile(null); setFileKey(k => k + 1); load();
    } catch (e) { alert('Erreur : ' + e.message); } finally { setBusy(false); }
  };
  const remove = async (id) => {
    ...
    try {
      const token = await getToken();
      await fetch(`${apiUrl}/api/admin/teachers/${teacherId}/documents/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
      load();
    } catch (e) { alert('Erreur : ' + e.message); }
  };
  return (
    <div className="border-t pt-3">
      <h4 className="font-medium mb-2 inline-flex items-center gap-1"><FileText className="w-4 h-4 text-gray-500" /> Documents (CIN, diplôme, contrat…)</h4>
      <div className="flex flex-wrap items-end gap-2 mb-3">
        <select value={docType} onChange={e => setDocType(e.target.value)} className="px-2 py-2 border border-gray-300 rounded-lg text-sm">
          {TEACHER_DOC_TYPES.map(([k, l]) => <option key={k} value={k}>{l}</option>)}
        </select>
        ...
        <input key={fileKey} type="file" accept="image/*,application/pdf" onChange={e => setFile(e.target.files?.[0] || null)} className="text-xs" />
        <button onClick={add} disabled={busy} className="flex items-center gap-1 px-3 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50"><Upload className="w-4 h-4" /> Ajouter</button>
      </div>
      <div className="space-y-1.5">
        ...
        {docs.map(d => (
          <div key={d.id} className="flex items-center gap-2 text-sm bg-gray-50 rounded-lg px-3 py-2">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full bg-white border border-gray-200 text-gray-600 shrink-0">{(TEACHER_DOC_TYPES.find(([k]) => k === d.doc_type) || [, 'Autre'])[1]}</span>
            <span className="text-gray-700 truncate flex-1">{d.label || 'Document'}</span>
            {d.view_url && <a href={d.view_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:text-blue-800 inline-flex items-center gap-1 text-xs"><ExternalLink className="w-3 h-3" /> Voir</a>}
